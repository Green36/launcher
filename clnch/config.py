...
def configure(window):
    ...
    def boot_AfxwL(info):
        filer = r"C:\TOOLS\launcher\afxw\AFXW.EXE"
        arg = '-s -l"%s/"' % info.args[0]
        directory = r"C:\TOOLS\launcher\afxw"
        shellExecute( None, filer, arg, directory )

    window.cmd_keymap[ "C-H" ] = boot_AfxwL

    def boot_AfxwR(info):
        filer = r"C:\TOOLS\launcher\afxw\AFXW.EXE"
        arg = '-s -r"%s/"' % info.args[0]
        directory = r"C:\TOOLS\launcher\afxw"
        shellExecute( None, filer, arg, directory )

    window.cmd_keymap[ "C-L" ] = boot_AfxwR
    ...
    # --------------------------------------------------------------------
    # テキストエディタを設定する
    window.editor = "C:/TOOLS/vim/gvim.exe"
    ...
    window.launcher.command_list += [
        ...
        ( "afxw",        window.ShellExecuteCommand( None, "C:/TOOLS/launcher/afxw/AFXW.EXE", "-s", "" ) ),
        ( "bin edit",    window.ShellExecuteCommand( None, "C:/TOOLS/launcher/Stirling/Stirling.exe", "%param%", "" ) ),
        ( "vim",         window.ShellExecuteCommand( None, "C:/TOOLS/vim/gvim.exe", "", "" ) ),
        ( "cygwin",      window.ShellExecuteCommand( None, "C:/cygwin/Cygwin.bat", "", "" ) ),
        ( "Translation", window.UrlCommand( "https://translate.google.co.jp/?hl=ja", encoding="utf8" ) ),
